...
class RandomLabel(Unlearner):
    ...
    def calculate_cost(self, train_data, reduction="mean"):
        ...
        y_forget = torch.randint_like(y_forget, low=0, high=rmodel.n_classes)
        ...
        fg_loss = nn.CrossEntropyLoss(reduction="none")(logits[:batch_size_forget], y[:batch_size_forget])
        rt_loss = 0
        if self.retain_lambda > 0:
            rt_loss = nn.CrossEntropyLoss(reduction="none")(logits[batch_size_forget:], labels[batch_size_forget:])
